...
@njit
def update_elec_field(start, end, hy, ez, ceze, cezh, ez_old):
...
@njit
def update_imp_ez_ADE(start, end, hy, ez, ez_old, hy_old, coef_d, c_a, c_b, d_a, coef_jj, coef_je, calc_denom, coef_a, coef_c, coef_b, pol_current, imp0, courant, alpha_b): # <--- Añadido alpha_b        
...
@njit
def update_imp_hy(start, end, hy, ez, ez_old, hy_old, chyh, chye, chye0):
        for m in range(start, end - 1):
            hy[m] = chyh * hy[m] + (chye/2) * (ez[m + 1] - ez[m] + ez_old[m + 1] - ez_old[m])   

        hy[end - 1] = chyh* hy[end - 1] + chye0 * (ez[end] - ez[end - 1])
